...
export const deploySharedContracts = async (clients, deployer, args, logger)=>{
    const txHashes = [];
    ...
    /* -------------------------------------------------------------------------- */ /*                          CHEAT CODES START HERE                            */ /* -------------------------------------------------------------------------- */ const feeAssetHandlerAddress = await deployer.deploy(l1Artifacts.feeAssetHandler, [
 ...
 */ export const deployRollupForUpgrade = async (clients, args, registryAddress, logger, txUtilsConfig)=>{
 ...
 */ export const deployRollup = async (clients, deployer, args, addresses, logger)=>{
    ...
    if (args.initialValidators) {
        await cheat_initializeValidatorSet(clients, deployer, rollupAddress.toString(), addresses.stakingAssetAddress.toString(), args.initialValidators.map((v)=>v.toString()), args.acceleratedTestDeployments, logger);
    }
...
export const handoverToGovernance = async (clients, deployer, registryAddress, governanceAddress, logger, acceleratedTestDeployments)=>{
    // We need to call a function on the registry to set the various contract addresses.
    const registryContract = getContract({
 ...
 * @param logger - The logger.
 */ // eslint-disable-next-line camelcase
export const cheat_initializeValidatorSet = async (clients, deployer, rollupAddress, stakingAssetAddress, validators, acceleratedTestDeployments, logger)=>{
 ...
 * @param logger - The logger.
 */ // eslint-disable-next-line camelcase
export const cheat_initializeFeeAssetHandler = async (clients, deployer, feeAssetAddress, logger)=>{
 ...
 */ export const deployL1Contracts = async (rpcUrls, account, chain, logger, args, txUtilsConfig = getL1TxUtilsConfigEnvVars())=>{
    ...
    const { feeAssetAddress, feeAssetHandlerAddress, stakingAssetAddress, stakingAssetHandlerAddress, registryAddress, rewardDistributorAddress } = await deploySharedContracts(clients, deployer, args, logger);
    const { rollup, slashFactoryAddress } = await deployRollup(clients, deployer, args, {
