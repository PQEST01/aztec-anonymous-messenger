...
declare type FilterOutAttributes<Base> = {
    [Key in keyof Base]: Base[Key] extends (...any: any[]) => any ? Base[Key] : never;
};
declare type PromisifyFunction<F extends (...any: any[]) => any> = (...args: Parameters<F>) => Promise<ReturnType<F>>;
declare type Promisify<Base extends {
    [key: string]: (...any: any[]) => any;
}> = {
    [Key in keyof Base]: ReturnType<Base[Key]> extends Promise<any> ? Base[Key] : PromisifyFunction<Base[Key]>;
};
declare type TransferTypes<Tuple extends [...args: any]> = {
 ...
 */
declare type MakeFunctionTransferrable<TFunction extends (...args: any) => any> = (...args: TFunction extends (...args: infer P) => any ? TransferTypes<P> : never) => ReturnType<TFunction>;
declare type Transferrable<Base extends {
    [key: string]: (...any: any[]) => any;
}> = {
    [Key in keyof Base]: MakeFunctionTransferrable<Base[Key]>;
};
export declare type Proxify<T> = Promisify<Transferrable<FilterOutAttributes<T>>>;
